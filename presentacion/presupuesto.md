...
Fuente de la slide obligatoria de presupuesto (`project-constraints.md`, «Slide de presupuesto y coste»). Es un borrador: **el coste de tokens es una estimación** hasta que la spec 017 lo mida en Langfuse sobre ejecuciones reales; entonces se sustituyen las filas marcadas con ⚠ y se recalcula el resto con las mismas fórmulas.

Moneda: euros, con 1 USD = 0,90 € (supuesto; se fija el tipo del día al cerrar la slide). Precios de modelo: los de OpenRouter consultados el 2026-09-23.
...
|---|---|---|
| Modelo de los roles que escriben y juzgan (entrevistador, planner, writer, crítico, editor, juez) | `anthropic/claude-sonnet-5`: 2 $/M de entrada, 10 $/M de salida, 0,20 $/M de lectura de caché y 2,50 $/M de escritura de caché | El modelo de cada rol aún no está fijado (`architecture.md` §15.2) |
| Modelo de los roles que extraen, registran o navegan (extractor, registrador, revisor visual) | `anthropic/claude-haiku-4.5`: 1 / 5 / 0,10 / 1,25 $/M | Tareas de comprensión, no de escritura |
...
| Herramientas: Claude Code Enterprise (~140 €/mes por puesto × 0,43 meses), OpenRouter de desarrollo y evals, portátil amortizado | | | 150 € |
...
Unsure: el precio del puesto de Claude Code Enterprise (sale de la factura de la empresa) y el gasto real de OpenRouter durante el desarrollo (sale de su página de actividad).
...
1. El coste real por novela y por revisión, en Langfuse (spec 017), contrastado una vez con lo que factura OpenRouter.
