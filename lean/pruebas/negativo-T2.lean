...
def cronologia : Cronologia where
  ...
  eventos := [
    ...
    { id := 61, momento := ⟨3200, 5, 14, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [⟨11, some 11⟩], tipo := .ordinario, analepsis := false },
    { id := 63, momento := ⟨3210, 1, 1, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [⟨11, some 25⟩], tipo := .ordinario, analepsis := false }
  ]
