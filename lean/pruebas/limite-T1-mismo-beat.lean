...
def cronologia : Cronologia where
  ...
  eventos := [
    ...
    { id := 61, momento := ⟨3226, 4, 2, 12, 0⟩, capitulo := some 3, beat := some 1, lugar := 21, presentes := [], tipo := .ordinario, analepsis := false },
    { id := 62, momento := ⟨3226, 4, 1, 12, 0⟩, capitulo := some 3, beat := some 1, lugar := 21, presentes := [], tipo := .ordinario, analepsis := false }
  ]
