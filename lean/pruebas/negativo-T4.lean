...
def cronologia : Cronologia where
  ...
  eventos := [
    ...
    { id := 61, momento := ⟨3226, 4, 1, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [], tipo := .excluyente 13, analepsis := false },
    { id := 63, momento := ⟨3226, 5, 1, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [⟨13, none⟩], tipo := .ordinario, analepsis := false },
    { id := 64, momento := ⟨3226, 6, 1, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [⟨13, none⟩], tipo := .ordinario, analepsis := false }
  ]
