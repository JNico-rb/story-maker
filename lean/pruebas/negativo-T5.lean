...
def cronologia : Cronologia where
  ...
  eventos := [
    ...
    { id := 62, momento := ⟨3185, 1, 1, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [⟨11, none⟩], tipo := .ordinario, analepsis := false },
    { id := 64, momento := ⟨3189, 1, 1, 12, 0⟩, capitulo := none, beat := none, lugar := 21, presentes := [⟨11, none⟩], tipo := .ordinario, analepsis := false }
  ]
