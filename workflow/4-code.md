...
- Test names state the behaviour, not the function. Each test carries its requirement's ID: `@pytest.mark.rf("RF-…")` in Python, the ID at the start of the test name in Vitest.
- A red test goes green through code. Never disable, skip or weaken a test for a green run.
- A lane branch reaches V2-test only through the orchestrator's `/integrar`, after the `verificador` subagent passes it.
...
4. Process records: add the rows the work produced, all in `verification.md`. A lane sends them to the orchestrator, which writes them when it integrates.
