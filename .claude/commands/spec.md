---
description: Escribe o cambia una spec — un redactor por spec, autorevisión, bloque en TODO.md por el integrador y auditoría a gap cero.
...
5. Lanza el subagente `auditor` con NNN, `spec` y ronda 1. HUECOS → `redactor-specs` con los huecos → `auditor` en la ronda siguiente. ESCALAR (ronda 3) → para y pasa al usuario los huecos y la decisión que falta.
