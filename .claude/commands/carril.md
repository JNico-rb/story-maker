...
3. **Tu cola.** De la tabla de carriles de `git show V2:TODO.md`: tus specs en orden y sus dependencias. La spec 000 tiene que estar cerrada en V2; si no, para: esperas a la 000. Una spec está **cerrada** cuando sus tres casillas de cierre están `[x]`; sus pasos D siguen `[ ]` con `(D, al final)` y no la bloquean.
4. Por cada spec de tu cola sin cerrar, en orden:
   ...
   4. **TDD.** Sigue `/implementar NNN` en esta sesión, o delega en el subagente `implementador` si el contexto se llena: paso a paso: prueba que falla por el motivo correcto → código mínimo → suite completa → `[x]` → commit. Las pruebas T usan siempre los dobles. **Los pasos D no se ejecutan**: quedan `[ ]` con `(D, al final)` y van en un lote cuando el backend esté completo (decisión del usuario, 2026-09-24); nada de modelo real ni de demostraciones antes.
