...
5. **Specs y planes por delante.** Para la siguiente spec sin aprobar de cada carril (y las que la siguen, si hay tiempo): `/spec NNN` y `/plan NNN` aquí, en V2. Las decisiones que cierre el redactor van a `architecture.md` §18. Specs de frontend (`specs/frontend/`, desde 022, carril E): escríbelas cuando estén aprobadas las specs de backend cuya API consume cada pantalla, y añade su fila a las tablas de `TODO.md` y a la de propiedad de `frontend/AGENTS.md`.
