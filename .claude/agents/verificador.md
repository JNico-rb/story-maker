...
2. Casos: cada paso `[x]` tiene al menos una prueba cuyo nombre dice su comportamiento, y esa prueba afirma la salida del caso y, si la spec los fija, su rechazo y su límite. Cada caso e invariante de clase T tiene paso y prueba. Los pasos D quedan `[ ]` con `(D, al final)` y los marcados `(recortado)` quedan `[ ]` con esa marca: ninguno impide el PASS (decisiones del usuario, 2026-09-24; recorte en `AGENTS.md` y en `TODO.md` → *Estado* → *Alcance*). Regla de cierre del recorte: bastan los casos C y los invariantes I que mapean a TLA+ (ReanudacionSinDuplicarNiPerder, ReintentosAcotados, VersionAnteriorConservada, atomicidad) o que protegen un validador; el resto de invariantes I va «(recortado)» y no bloquea. Cualquier otro paso sin `[x]` es FAIL.
...
6. Spec y docs: si el código hace algo que la spec no dice, o un paso `[x]` dice algo que el código no hace, es FAIL (lo recortado y lo D no cuentan; no retraces la spec entera); si el carril declaró cambios de spec o de docs, compruébalos.
