...
2. **Nada aprobado se modifica**: biblia, escaleta de alto nivel, escaletas de arco validadas, capítulos aprobados, manuscrito, y el libro de estado salvo al cerrar un capítulo. El harness crea cada uno de esos ficheros con `Write` una sola vez; `.claude/hooks/inmutables.sh` bloquea `Edit` sobre ellos siempre, y `Write` cuando el fichero ya existe (spec §3.1). El libro de estado es la excepción: su `Write` pasa, porque se sustituye entero al cerrar cada capítulo.
