...
- **`characters`, `places`, `facts`**: solo lo que inventas. **Toda fecha, año, edad, cifra o antigüedad que la trama necesite** (la historia de un objeto, el año de un suceso pasado, cuánto hace de algo) es un hecho inventado del plan, con su valor exacto y coherente con los demás, y lo citan todos los beats que la usan: ningún capítulo tiene que inventarla. Cada hecho inventado lleva un `id` propio del plan (por ejemplo `p1`) para que los beats lo citen, y su sujeto es un personaje o un lugar por su nombre, o `world`.
