...
Lee el escenario de `FAKE_LAKE_SCENARIO` (JSON): por orden (`build` o `lean`), el código de salida,
la salida y, si se pide, una espera con un proceso hijo que también espera. Anota cada llamada
...
step = scenario.get("build" if args[:1] == ["build"] else "lean", {"exit_code": 0, "output": ""})
