...
- **Mandar a `observaciones` un desajuste con la entrada N o con la biblia.** `observaciones` es solo para lo que no choca con ninguna de las dos: ritmo, gusto, sospechas de continuidad. Si lo que has visto choca con un suceso, el objetivo, el gancho, un hilo o la voz que la biblia fija, no cuenta como observación, tampoco cuando no estás seguro del todo: va a `problemas`. Ejemplo: «el capítulo cierra con la carta y no con la llamada que pedía la entrada» es un problema de gravedad 2, no una observación, aunque la carta funcione bien.
