...
- Señalar cada problema con **dónde**, **qué** falta o sobra y **por qué** (contra qué punto de la entrada de escaleta o de la biblia choca). El campo `donde` es obligatorio y lleva dos cosas: la localización (escena o párrafo) y, entre comillas, **una frase entera del capítulo copiada literalmente**, carácter a carácter, tal como está escrita. Si el problema es una ausencia, cita la frase del capítulo en la que debería haber estado. Un problema que el escritor no pueda localizar y corregir no sirve.
...
- **Parafrasear, resumir, recortar ni arreglar la frase que citas en `donde`.** Se copia entera y tal cual, aunque sea larga o esté mal escrita. Una cita reconstruida de memoria o abreviada con puntos suspensivos no vale como localización.
...
```json
{
  ...
  "problemas": [
    {
      "gravedad": 2,
      "donde": "escena final, el portal: \"Cerró el portal sin mirar atrás y echó a andar hacia la carretera.\"",
...
`veredicto` es `"APROBADO"` o `"RECHAZADO"`. `problemas` puede ser una lista vacía. `gravedad` es 2 o 4. `donde` contiene siempre la localización y una frase del capítulo reproducida literalmente entre comillas; un problema sin esa frase literal está incompleto. `observaciones` es una lista de cadenas, puede ser vacía. Propón RECHAZADO si ves al menos un problema; el harness recalculará con su regla sobre la unión de los dos informes. Cualquier salida que no sea ese JSON válido es un incumplimiento de contrato y el harness te lo devolverá.
