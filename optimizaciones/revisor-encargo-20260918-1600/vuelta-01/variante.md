...
- **Recorrer la escaleta como procedimiento, no por impresión general.** No juzgues el capítulo por cómo se lee: comprueba las listas, en este orden y sin saltarte ninguno de los cuatro pasos.

  1. **Extrae la entrada N** y escribe, antes de valorar nada, la lista de lo que fija: cada suceso clave, el objetivo narrativo y el gancho de cierre.
  2. **Recorre esa lista punto por punto.** De cada punto decide «está» o «no está» en el texto del capítulo, y si está, anota dónde lo ves (escena o cita breve). Cada punto que no esté es un problema de gravedad 2, uno por punto.
  3. **Compara el cierre del capítulo con el gancho que fija la entrada N.** No basta con que el capítulo termine en suspense: tiene que terminar en *ese* gancho. Si cierra con otro, lo invierte o lo resuelve en vez de dejarlo abierto, es un problema de gravedad 2.
  4. **Recorre las entradas posteriores a N, una por una**, y de cada una, cada uno de sus sucesos clave. De cada suceso posterior busca en el capítulo si ya se cuenta como ocurrido, o como ya sabido por un personaje o por el narrador. Basta una aparición, aunque sea de pasada, en un diálogo, en un recuerdo o dada por supuesta: es un adelanto, y es un problema de gravedad 2 que debe citar la entrada posterior a la que ese suceso pertenece.
