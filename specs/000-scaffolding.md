...
- **Marca corporativa**, definida una sola vez como tokens del tema y aplicada por la cabecera de la aplicación (logotipo y nombre «Qaracter · Story Maker»), que la ruta raíz muestra mientras no haya pantallas. Logotipo: `images/qaracter-logo.png`. Tipografías instaladas como paquetes, sin CDN. Tokens:
...
- **Formal:** `lean/`, proyecto Lake mínimo con su toolchain fijada, que compila en la CI (Lean no corre en el portátil, ADR 0004); `tla/`, carpeta de las especificaciones, con TLC en versión fijada sobre Temurin en la CI.
...
- `config.json`, ajustes del servidor y contenido de `.env.example`, esquema SQLite (con la carga de `sqlite-vec` y FTS5), API mínima, subórdenes `serve`, `init-db` y `check-env`, valor por defecto del directorio de datos y README → 001-base.
...
Excepción de la 000: el comportamiento observable son las órdenes que ejecuta un desarrollador y lo que producen, así que se nombran órdenes, rutas y herramientas. Un caso es **T** si una prueba o un job de la CI lo ejecuta con entradas fijas y decide solo; es **D** si necesita un push, una sesión real de Claude Code o un navegador.
...
- **Dado** el repositorio con un `.gitattributes` que declara `* text=auto eol=lf`
...
- **Entonces** responde lo de la tabla: *deniega* con código 2 y un motivo que nombra la ruta y lo que falta; *deja pasar* con código 0. Rutas guardadas, la puerta «Write tests or code» de `AGENTS.md`: `backend/src/**`, `backend/tests/**`, `frontend/src/**`, `frontend/tests/**`, `lean/**`, `tla/**` y `.github/workflows/**`. Un bloque va de su `## NNN — <nombre>` al siguiente `## `; tiene el plan aprobado si contiene la línea `- [x] Plan below approved`, con o sin sufijo; un paso pendiente es un `- [ ]` bajo su `### Steps`, nunca bajo `### Closing`.
...
| `docs/…`, `specs/…`, `TODO.md`, `README.md`, `.claude/…`, una ruta fuera del proyecto | cualquiera | Deja pasar |
...
- **Entonces** registra `guard-secretos` y `guard-plan` en `PreToolUse` para las tools `Edit`, `Write` y `MultiEdit`, invocados con la ruta de la raíz del proyecto (no relativa al directorio actual); deniega `Read(.env)`, `Read(**/.env)`, `Read(.claude/settings.local.json)` y `Bash(git push --force*)`; permite `uv`, `pnpm.cmd`, `node`, `java` y git de lectura, commit, worktree, merge y rebase (`verification.md` §9.6)
...
- **Cuando** se ejecutan las órdenes de 000-C03 y 000-C06 y las pruebas de los hooks (`node --test`)
- **Entonces** todo sale en verde con `pnpm.cmd` y Python 3.12, sin pasos fuera de esas órdenes; el clon tiene `lean/`, `tla/`, `ejemplos/briefs/`, `presentacion/`, `images/qaracter-logo.png`, `frontend/` y el esqueleto del backend; ninguna orden avisa de conversiones LF→CRLF; después, `git status --porcelain` no lista nada. El clon se borra al terminar
...
- **Dado** `.claude/settings.json` activo y una sesión nueva de Claude Code en la raíz
- **Cuando** se le pide, por turnos: leer `.env`; escribir en un fichero de prueba fuera de las rutas guardadas una clave falsa con la forma de 000-C09 y un marcador de prueba evidente (por ejemplo, «TEST» repetido); lo mismo tras `cd backend`; `git push --force --dry-run`; `uv run pytest` desde `backend/`; y, en un worktree temporal de una rama desechable cuyo `TODO.md` no tiene ningún plan aprobado con pasos pendientes, escribir en `backend/tests/`
- **Entonces** la lectura de `.env` se deniega sin mostrar nada de su contenido; las dos escrituras con la clave se bloquean con el motivo de 000-C09; `git push --force` se deniega sin ejecutarse; `uv run pytest` corre sin pedir permiso; la escritura del worktree temporal se bloquea con el motivo de 000-C10. Se borran el worktree y la rama; resultado en `verification.md` §9.6
...
- **Cuando** corre la CI
- **Entonces** terminan en verde, en Linux, los jobs de `verification.md` §4.6: `backend` (`uv sync --frozen`, `ruff check`, `ruff format --check`, `mypy src`, `pytest`); `frontend` (`pnpm install --frozen-lockfile`, typecheck, lint, test, build); el formal, en uno o en dos jobs (`lake build --wfail` de `lean/` con su toolchain fijada; `tla2tools.jar` de una versión fijada, nunca «latest», sobre Temurin: TLC arranca e informa de su versión y, con `tla/` sin especificaciones, lo dice en el registro y no falla); y `seguridad` (`detect-secrets` sin hallazgos, bloqueante; `pip-audit` y `pnpm audit` informan sin cambiar el resultado del job). Ningún job usa secretos del repositorio
...
| 000-I5 | Los hooks usan solo la biblioteca estándar de Node (sus pruebas corren con `node --test` sin instalar nada) y deciden igual con el directorio actual en un subdirectorio | T | Las tablas de 000-C09 y 000-C10 repetidas con el directorio actual en `backend/` |
...
- `architecture.md` §14.8 (frontend, marca, proxy, tipos generados), §15.1 (stack), §15.3 (entorno Windows), §15.5 (`LANGFUSE_MCP_AUTH`, solo desarrollo), §15.9 (organización del backend y regla de dependencias), §16.20 (Claude Code en el desarrollo), §18 (organización del frontend, cliente de la API, dónde corre Lean); ADR 0004.
- `verification.md` §2 (clases), §3.1–§3.3 (tipos, análisis estático, regla de dependencias), §3.7 (sin contratos de importación), §4.6 (CI), §4.10 (herramientas de TLC), §5 (filas R.10, R.12, R.13, P.1), §9.2 (servidores MCP), §9.3 (log del browser MCP), §9.5 (espejo de memoria), §9.6 (hooks y permisos).
- `definitions.md` §11.3 (ajustes del servidor; `LANGFUSE_MCP_AUTH` no es del producto).
- `AGENTS.md` (puerta «Write tests or code», formato de `TODO.md`), `CLAUDE.md` raíz (órdenes canónicas, trampas del entorno, secretos), `backend/AGENTS.md` y `frontend/AGENTS.md` (stack, órdenes, módulos, propiedad de la 000, marca).
- `project-constraints.md`, «Los repositorios deben incluir»: sin API keys (000-C01, 000-C09, 000-I4), `.claude/` commiteada con la memoria (000-C01, 000-C12), config MCP con un browser MCP (000-C15, 000-C16); «Presentación»: imagen corporativa con `images/qaracter-logo.png` (000-C07, 000-C08, 000-C16); §5c y §5d: `lake build` y TLC con la config en el repo, como herramienta (000-C18).
...
| ¿Cuándo es T una orden? | Cuando una prueba o un job de la CI la decide solo; push, sesión real o navegador → D | `verification.md` §2, §4.6 (la CI es T) |
| ¿Qué valores tiene la marca? | Primario y secundario muestreados del logotipo; fondo, texto y acento derivados; Inter Tight y Literata | **Hueco del doc**: §14.8 solo pide tokens → decisión para §18 |
| ¿Tipografías por CDN? | No: empaquetadas; ningún tercero ve al lector y la SPA no depende de red externa | **Hueco del doc** → decisión para §18 |
| ¿Página de inicio? | No hay pantalla de inicio en los docs: la ruta raíz muestra la cabecera de marca de la aplicación, que reutilizarán las pantallas; la 000 no posee slices de `pages/` | `architecture.md` §14.8, `frontend/AGENTS.md` |
| ¿Qué rutas guarda `guard-plan`? | Toda la puerta «Write tests or code»: `backend/src`, `backend/tests`, `frontend/src`, `frontend/tests`, `lean`, `tla`, `.github/workflows` | Orquestador y `AGENTS.md`; **contradice `verification.md` §9.6**, que solo guarda `src/**`: actualizarla antes de aprobar |
| ¿Y los manifiestos y `backend/harness_workspace/`? | Quedan sin guardar, como decidió el orquestador | Propuesta: guardarlos también |
...
| ¿Uno o dos jobs formales? | §4.6 dice `formal`; se admite partirlo en Lean y TLC | `verification.md` §4.6 |
| ¿Directorio de datos por defecto? | Se ignora `data/` en la raíz; 001 debe usarlo por defecto o avisar al integrador | **Hueco del doc**: §15.5 no da valor por defecto |
...
| ¿Finales de línea? | LF en el índice; binarios sin conversión | Orquestador; **hueco del doc** (§15.3) |
| ¿Cubre `Bash(git push --force*)` también `-f` y `+rama`? | No; la spec sigue §9.6 | **Hueco del doc**: ampliar la regla |
| ¿Escrituras por Bash (`node -e`, redirecciones)? | No pasan por `guard-secretos`; las caza `detect-secrets` en la CI | **Hueco del doc**: riesgo para `verification.md` §6 |
| ¿Cita §5 la 000? | No; R.10, R.12, R.13 y P.1 la tienen ahora | **Hueco del doc**: añadirla en §5 |
