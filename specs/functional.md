...
- **Nada aprobado se modifica**: ni la biblia ni la escaleta de alto nivel tras la aprobación del usuario, ni la escaleta de un arco tras validarla el harness, ni un capítulo tras el veredicto APROBADO (o la aceptación por agotamiento), ni el libro de estado salvo por el harness al cerrar un capítulo. El harness crea cada uno de esos ficheros con una escritura completa, una sola vez (o una por versión, en la biblia y la escaleta antes de aprobarse); nunca los edita en sitio. En el hito 1 lo impone un hook `PreToolUse` sobre `Edit` y `Write` ([`.claude/hooks/inmutables.sh`](../.claude/hooks/inmutables.sh), registrado en `.claude/settings.json`), que cubre `biblia.md`, `escaleta.md`, `arco-*.md`, `informe-arco-*.md`, `intento-*.md`, `libro-estado.md`, `libro-estado-*.md` y `manuscrito.md` dentro de `novelas/`. Bloquea **siempre** `Edit`, porque todos se sustituyen enteros y ninguno se edita nunca en sitio. Para `Write` hay dos reglas, según el fichero se escriba una sola vez o una por versión:
  - **Por aprobación**, para `biblia.md`, `escaleta.md` y `arco-*.md`: el hook lee el frontmatter del fichero que **ya existe** y bloquea si trae `aprobada: true` (biblia y escaleta) o `validada: true` (escaleta de arco). Sin esas marcas deja pasar, porque una propuesta todavía no aprobada se reescribe a propósito: el bucle de `proponer_escaleta` la rehace en cada vuelta, y aprobarla consiste precisamente en reescribirla con la marca puesta (§4.1). Lo que el hook protege no es el fichero, es la aprobación.
  - **Por existencia**, para `intento-*.md`, `libro-estado-*.md`, `informe-arco-*.md` y `manuscrito.md`: se escriben una sola vez en la vida, así que el primer `Write` es el harness creando el artefacto —legítimo— y el segundo es el error que hay que parar.

  `libro-estado.md` es la excepción declarada a las dos reglas: su `Write` siempre pasa, porque el harness lo sustituye entero al cerrar cada capítulo; su `Edit` no, porque nunca se edita en sitio. El hook **falla en abierto**: si no logra leer de su entrada la herramienta o la ruta, o no logra leer el fichero existente para mirar su frontmatter, deja pasar, porque uno que bloquea sin entender su entrada pararía el bucle sin motivo. **Limitación conocida:** la garantía del hook cubre `Edit` y `Write`, **no Bash**. `cerrar_capitulo` copia `libro-estado-K.md` sobre `libro-estado.md` con `cp`, y filtrar comandos de shell por ruta exigiría un parser nuevo dentro del bucle; un `cat > fichero` o un `sed -i` sobre un artefacto aprobado pasarían sin que el hook se entere. Por eso el criterio 6 de §8.2 sigue comprobando la inmutabilidad con git a posteriori: el hook previene lo que alcanza, el criterio demuestra lo que de verdad pasó. En el hito 2 la inmutabilidad es código y la limitación desaparece.
...
6. **Las credenciales tampoco se leen desde la sesión.** `.claude/settings.json` lleva dos reglas `deny` —`Read(.env)` y `Read(.claude/settings.local.json)`— que niegan la lectura de los dos ficheros donde viven las claves. No entrar en el repositorio impide publicarlas; esto impide leerlas, que es distinto: los cinco subagentes se definen con `tools: Read, Glob, Grep` (§3.1) y cualquiera de ellos alcanzaría esas rutas. Un `deny` de `Read` cubre además `cat`, `head`, `sed` y las redirecciones de Bash sobre esas mismas rutas, así que no hace falta un hook de Bash aparte para lo mismo. Quien sí las necesita —el hook de sesión y el exportador— las recibe del entorno, no de una lectura de fichero.
