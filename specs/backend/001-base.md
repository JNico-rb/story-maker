...
| `role_sessions` | ejecución, o novela fuera de ella | `novel_id`, `run_id?`, `role`, `chapter?`, `model`, `prompt_version?`, `reserved_tokens` (la reserva del `TechoDeTokens` al abrir la sesión, `architecture.md` §6.5.1; la calcula 003, esta spec solo declara la columna), `input_tokens?`, `output_tokens?`, `cache_read_tokens?`, `cache_write_tokens?`, `cost_usd?` (vacíos, nunca a cero, cuando la sesión cierra sin resultado final: `architecture.md` §18), `sdk_cost_usd?` (el `total_cost_usd` del SDK, solo como contraste, `architecture.md` §13.2), `latency_ms`, `outcome`, `trace_id?` |
