...
| ¿Son de la 003 los schemas de las tools de cada rol? | No: su contenido es del dominio de cada rol y su spec lo declara con este mecanismo. La 003 exige que cuadren con la lista blanca | Tabla de propiedad de `backend/AGENTS.md`; **decisión para §18** |
...
| ¿Qué campos escanea la política? | Los que cada tool marca como narrativos, con su ruta; nunca listas de prohibidas ni léxico a evitar | `architecture.md` §7.5; **decisión para §18** |
...
| ¿Se guarda la reserva? | Sí, con la `SesionDeRol` | `architecture.md` §6.5.5; **hueco**: falta en `definitions.md` §5 y en `role_sessions` (§15.6) |
| ¿Dónde va el coste que declara el SDK? | Solo en la `LlamadaDeModelo`, como contraste | `architecture.md` §13.2; **decisión para §18** |
| ¿Uso de una sesión sin resultado final? | Vacío, no cero: cero ocultaría un consumo real | Premisa 5 (`architecture.md` §2); **decisión para §18** |
...
| ¿Y las `ANTHROPIC_*` heredadas con `claude_login`? | No llegan con valor: una clave suelta cobraría créditos de API | `verification.md` §4.7; **tensión** con «no toca `ANTHROPIC_*`» de §15.2 |
| ¿`ANTHROPIC_API_KEY` en `anthropic_compatible` sin OpenRouter? | Vacía siempre: solo las variables del endpoint | `verification.md` §4.7; **decisión para §18** |
...
| ¿Y si la política falla? | La tool no corre; la sesión se corta y el error sube | Cierre a fallo de §7.5; **decisión para §18** |
...
| ¿Pico de tokens concurrentes de las evals? | Fuera: es de 020, con las reservas de la `SesionDeRol`; le falta el momento de apertura | `verification.md` §4.2 (b); **hueco** |
