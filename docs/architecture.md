...
- el `PuntoDeControl` del capítulo n.
...
| `Planificar` | `queued` → `planning` | Worker: toma la primera de la cola y abre el planner |
| `Regenerar` | `queued` → `writing` de los capítulos afectados | Worker: revalida la versión base, copia la candidata, aplica los hechos |
...
- `checkpoints` guarda una fila por capítulo aceptado (0 = plan aplicado). Solo inserción.
...
En el capítulo editado a mano, las puntuaciones del editor se registran pero no bloquean: bloquean los validadores deterministas y el gate. Si un fallo del gate se atribuye al capítulo editado, la ejecución termina `failed` (`edit_rejected`) con los defectos: un rol no corrige lo que una persona escribió a propósito. La persona lo arregla y vuelve a guardar.
...
- **Cómo se aplica.** Flujo en cinco capas (docs → specs → plan → tests → código). Subagentes: `redactor-specs`, `auditor` (aprueba spec y plan con gap cero), `implementador` (TDD), `verificador` (cierra) y `seguridad`. Comandos `/orquestar`, `/carril`, `/spec`, `/plan`, `/implementar`, `/integrar`, `/estado`. Cuatro carriles en worktrees hermanos que se integran en V2. Hooks de desarrollo que bloquean secretos y código sin plan aprobado.
