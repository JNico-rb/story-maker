...
- **Modelos provisionales** (§15.4): `claude-haiku-4-5` en todos los roles (usuario, 2026-09-25: cuota de la suscripción). Antes planner y juez iban en `claude-sonnet-5`, y el writer pasó a haiku el 2026-09-24. Se revisan en la iteración de tuning (§17).
...
| `revision-visual` | programática (browser MCP) | Gate | sí | 0/1 por portada, índice, capítulos, ficha | Portada, índice, capítulos y ficha renderizan y enlazan; fallo de datos → editor re-registra; de render → `failed`. Etapa 3 del gate desde 2026-09-25 (spec 017: el revisor visual navega la vista con Playwright MCP y el código compara; sin navegador, la ejecución se interrumpe) |
...
| `operation.roles.<rol>.model` | `claude-haiku-4-5` en todos los roles | Provisionales |
...
| Modelo por rol (`roles.<rol>.model`) | Calidad y coste (a precio de lista) medidos en las evals; hoy `claude-haiku-4-5` en todos |
...
| Modelos por rol | Uno para todos · uno grande donde se escribe o juzga la novela entera y uno ligero en el resto | Calidad donde se nota; menos consumo del límite de uso | `claude-haiku-4-5` en todos los roles (cuota, usuario 2026-09-25). Provisional (§17) |
...
| Alcance del frontend | SPA completa (acceso, mis novelas, entrevista, progreso, lectura, cambio) · lectura y cambio en la SPA, configuración por la CLI · solo PDF y CLI | El encargo pide en la web la lectura y el cambio desde la página, no la configuración; plazo; menos deriva que quitar la web | SPA con acceso (022), «mis novelas» mínima (023: lista con enlace a la lectura, destino del acceso; usuario, 2026-09-25), lectura (026) y cambio del lector (027); entrevista, confirmación del brief, lanzamiento y progreso por la CLI (§15.8). El resto de 023, 024, 025 y 028 fuera de alcance (usuario, 2026-09-24); **reabierta** (usuario, 2026-09-25, segunda decisión): SPA completa, con 023 entera, 024, 025 y 028, porque la web debe crear la novela y modificarla donde se quiera. En 027 se selecciona solo un fragmento: la lectura no expone los hechos y el encargo pide «un fragmento o un hecho»; la API admite los dos |
...
| Fuente de `report metrics` (030) | API de Langfuse · SQLite | El plan Hobby retiene poco y la evidencia no depende de Langfuse (§11.7, §13.2); `role_sessions` y `validator_results` ya guardan uso, coste, latencia, versión de prompt y scores | SQLite: `report metrics` agrega esas dos tablas y escribe `docs/metrics.md`; sin red, un dato que falta sale como hueco, nunca estimado (usuario, 2026-09-24) |
| Identidad de cada brief de eval en `evals table` (020) | Id de la novela · `Novel.title` con un slug fijo · el brief de eval de la novela | Filas estables entre ejecuciones; el planner reescribe `Novel.title` con el título del plan, así que el título no sirve (reabierta 2026-09-25) | El brief de eval de la novela (`definitions.md` §3 Novela): `evals run` y `example` lo guardan al importar un brief de `ejemplos/briefs/`; la tabla busca por él |
| Enlace entre la solicitud y las sesiones de su propuesta (014-C19) | Por la ejecución · por la traza de la propuesta guardada en la solicitud | La propuesta corre antes de que exista la ejecución; `SesionDeRol` ya guarda su traza | La `SolicitudDeCambio` guarda su traza de la propuesta (`definitions.md` §5); el coste de la revisión = sesiones de esa traza + sesiones de su ejecución |
| Prohibidas de nivel user de un brief de evals (020) | Nivel user del cliente de `--email` · un cliente por brief · nivel novela de la novela del brief | Cada brief representa a su propio cliente, pero los cinco comparten `--email`: en el nivel user, la lista de un brief (boda: «pantalla») se aplicaba a los otros cuatro y tumbó la ejecución 11 (`verification.md` §8, fila 9). Un cliente por brief dejaría la novela del brief 1 fuera de la cuenta de la revisión humana (usuario, 2026-09-25) | Nivel novela: `evals run` y `example` registrarán las `user_banned_terms` de un brief de `ejemplos/briefs/` como prohibidas de la novela de ese brief (020-C18, **pendiente de implementar**: el usuario lo aplazó el 2026-09-25; en la tanda D la lista de boda se aisló a mano, `verification.md` §8, fila 9) |
| Ejecución de un brief en `evals table` (020) | La primera de generación de su novela · la última | Tras una iteración de tuning (020-C11) la novela tiene una ejecución nueva; la tabla da el estado actual del brief, y el «antes» se conserva en §4.2 (c) (2026-09-25) | La última ejecución de generación de la novela del brief |
| `evals run` con un brief ya generado (020) | Generarlo otra vez · no repetirlo | La cuota de la suscripción: la novela de `example` sobre el brief 1 sirve de eval, y una tanda cortada por cuota se reanuda sin repetir (usuario, 2026-09-25) | Un brief de eval que ya tiene novela del cliente no crea otra: se nombra con la suya y, si su ejecución sigue en la cola o interrumpida, la procesa el mismo worker (020-C17) |
| «Pico de tokens concurrentes» del resumen por brief (020-C07) | Suma de reservas simultáneas · máximo individual | Una sola sesión de rol a la vez por ejecución (§6.5) | Máximo de `reserved_tokens` de las sesiones de la ejecución |
| Traza del resumen por brief (020-C07) | Todas · la primera | Una ejecución conserva su traza al reanudarse (§9) | La de la primera `role_session` de la ejecución; «—» si no hay |
