...
Con él, toda afirmación sobre el mundo debe poder responder a *¿de qué novum se deriva, y en cuántos pasos?* Así, una pregunta irrespondible («¿es plausible este futuro?») se convierte en otra que se puede verificar sobre un grafo: «¿es alcanzable esta consecuencia desde el novum?».
...
La coherencia especulativa es un grafo dirigido, no una lista de rasgos del mundo.
...
graph TD
    ...
    CON -->|deriva de| CON
    NOV --> RES[Restriccion<br/>lo que no puede ocurrir]
    CON --> RES
    CON --> HEC[Hechos del mundo<br/>tecnologia, gobierno, economia,<br/>norma social, entidad artificial]
    HEC --> EVT[Eventos de la trama]
    PER[Personajes<br/>destinatario y allegados] --> EVT
```
...
- **T6 · El mundo sigue a su novum.** Ningún evento que dependa de una consecuencia es anterior a la fecha del novum.
...
El tono también pesa —un tono épico admite frases más largas que uno divertido—, pero ese matiz no se fija en cifras: lo juzga la rúbrica (`architecture.md` §10.3). Un objetivo por franja y tono serían dieciocho combinaciones por calibrar.
...
- **El rigor especulativo** no se evalúa en abstracto: se comprueba la trazabilidad dentro del grafo causal de la obra. ¿Deriva esta consecuencia del novum? Es una pregunta verificable, y su respuesta correlaciona razonablemente con lo que un lector percibe como un mundo bien construido.
- **La calidad narrativa** se juzga con una rúbrica de criterios concretos, cada uno con escala y justificación (`architecture.md` §10.3). La rúbrica se calibra contra el juicio humano. Un juez cuya puntuación no se parece a la de una persona que lee la misma novela no mide lo que dice medir.
