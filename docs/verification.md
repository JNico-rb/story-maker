...
- **004-C14, comprobado 2026-09-25 (ejecución 18, novela 9, `mcp__langfuse__*` de solo lectura):** `sessionId="9"` agrupa las trazas de la novela (una por llamada MCP: `submit_plan`, `submit_chapter`, `submit_review`, `submit_facts`…), con spans `rol:<rol>`, `tool:<nombre>`, `capitulo-<n>` y `validador:<nombre>` (incluido `validador:cronologia-lean`, que cruza con `validator_results` de la misma ejecución, §8 fila 17). Los `GENERATION` (`llamada:claude-sonnet-5`, `llamada:claude-haiku-4-5`) traen coste y tokens por llamada (p. ej. 0,576 USD / 109.031 tokens la del planner); agregando por `sessionId=9`: 38 llamadas, 3,17 M tokens, 4,31 USD (`claude-haiku-4-5` + `claude-sonnet-5`). Los scores llegan con nombre y comentario (`rubrica-capitulo/fidelidad-canon`, `nombres-exactos`, `longitud-capitulo`, `schema-salida`…), con personajes de la novela (Iker, Nerea, Chus, Pancho), no del cliente real. **Sin confirmar:** el `input`/`output` de cada observación (GENERATION y TOOL) llega `null` a esta API — no se pudo ver el brief enmascarado en sí, solo que nunca viaja en crudo (cumple 004-I1 por omisión, no por máscara visible).
...
2. `story-maker evals run` importa cada brief (mismo schema y comprobaciones que la entrevista; los hechos extraídos del texto libre se aceptan sin cliente) y lanza su ejecución de generación con la config y la etiqueta de prompts vigentes. Corre **solo en la máquina con sesión de Claude Code iniciada** (`LLM_PROVIDER=claude_login`, suscripción de la organización); nunca en CI. Un brief que ya tiene novela del cliente (p. ej. la de `example` sobre el brief 1) no se genera otra vez (`architecture.md` §18). Las prohibidas de nivel user de un brief se registran a nivel de su novela, para que no se filtren a los demás briefs del mismo cliente (020-C18).
...
| U10 | **`nombres-exactos` imperfecto** | Un diminutivo fuera de la distancia admitida, o un nombre escrito entero en minúscula, escapa; una palabra corriente que solo sale a principio de frase, a distancia 1 de un nombre de 4 a 6 letras («Nada» por «Nala» sin ningún «nada» en el texto), dispara; si el texto la escribe también en minúscula, no (2026-09-25) | Distancia acotada por longitud y solo en palabras con mayúscula inicial (`architecture.md` §11.2); calibración con los 5 briefs; `coherencia-personajes` |
...
| 13 | 2026-09-25 | eval (ejecución 16) | `nombres-exactos` tomaba «Como»/«Cómo» a principio de frase por variante de «Cobo» (distancia 1, 4 letras) y rechazó 11 entregas; ahora no dispara una palabra con mayúscula que el mismo texto escribe también en minúscula. El motivo de fallo repetía seis veces el comentario del juez (un defecto por capítulo citado); ahora cada mensaje una vez. Prompts: toda fecha, edad o cifra de la trama es un hecho del plan; el writer no inventa ninguna y el editor las lleva al resumen (el juez tumbó la 16 por las fechas del roble 1926/1948/1972) | Los 10 capítulos de la 16 pasan la regla nueva sin variantes; efecto en generación: pendiente (ejecución 18) | `architecture.md` §11.2, §6 U10, 011-C12, 012 (bug D1), `planner.md`, `writer.md`, `editor.md` |
| 14 | 2026-09-25 | auditoría (segunda tanda, carril N) | Ninguna spec lanzaba la generación desde la web ni resolvía qué ejecución enseña el progreso: 008-C02 añade `latest_run_id` a la novela, 024-C21 «escribir la novela», 025-C00 progreso por la novela; alcance completo de la SPA reabierto (usuario) | La web crea, entrevista, lanza, sigue y edita a mano | `architecture.md` §18 «Alcance del frontend», 008, 023, 024, 025 |
| 15 | 2026-09-25 | entorno | Las pruebas de 027 caducaban solas: `expires_at` fijo a las 12:00 UTC del mismo día; y una fecha lejana desborda `setTimeout` (> 24,8 días) y caduca al instante | Caducidad relativa al reloj real; C12 conserva su reloj controlado; 76/76 | pruebas de 027 |
| 16 | 2026-09-25 | eval (ejecución 18, brief sintético, commit af68db6) | Primera ejecución con los arreglos de la 16: 10 capítulos y gate en 1 h 47 min, 4,30 USD; `failed` (retries_exhausted). El plan estaba en orden, pero el editor registró los eventos del capítulo 2 un día después de lo planificado y Lean (T1) lo cazó; la reescritura del capítulo 3 se pasó de largo (1.537 palabras), porque el writer v3 solo sabía alargar. Editor: el momento de un evento es el planificado salvo que el texto fije otro. Writer: si vuelve por larga, recorta hasta ~1.350; al reescribir, no alarga | pendiente | `editor.md`, `writer.md` |
| 17 | 2026-09-25 | GitHub Actions (007-C25, sobre la ejecución 18) | Verificación del tiempo de respuesta real del workflow `verificar-cronologia.yml` frente a `verifier_timeout_seconds` (900 s, `config.json`). `validator_results` de la ejecución 18 (fila 575) trae `cronologia-lean`, `passed=0`, con el defecto T1 «el evento del capítulo 3 (16 sept, 18:00) se narra después del capítulo 2 (16 sept, 20:00), pero ocurre antes» — el mismo defecto que anotó el juez en Langfuse (§4.1). La API pública de GitHub (sin `gh`, no instalado en esta máquina) muestra la corrida más cercana en el tiempo, `36153416418` (`workflow_dispatch`, job `lean`), de 15:19:44 a 15:20:20 UTC: 36 s de corrida, job en 29 s | 29–36 s frente a un timeout de 900 s: muy por debajo | `validator_results` (SQLite, solo lectura); `https://github.com/JNico-rb/story-maker/actions/runs/36153416418` |
...
**000-C17, comprobado 2026-09-25:** cargas JSON por stdin, como las invoca Claude Code (`tool_input`, `cwd`), no la sesión interactiva completa del caso. `guard-secretos`: una clave falsa (`sk-ant-` + `TEST` × 10, con marcador de prueba) en un `Edit` → bloqueado, exit 2, motivo «clave de Anthropic»; el mismo `Edit` con `TU_CLAVE_AQUI` → exit 0. `guard-plan`: un `Write` a `backend/src/foo.py` con `CLAUDE_PROJECT_DIR` apuntando a un `TODO.md` sin ningún bloque `- [x] Plan below approved` → bloqueado, exit 2; el mismo `Write` con `CLAUDE_PROJECT_DIR` en la raíz real (con planes aprobados y pasos D pendientes) → exit 0. `.claude/settings.json` deniega `Read(.env)` y `Read(**/.env)` (no se leyó `.env`). No se abrió una segunda sesión real ni se probaron `git push --force --dry-run` o `uv run pytest` desde una sesión interactiva.
