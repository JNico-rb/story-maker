...
- **Cuatro carriles** de Claude Code trabajan en paralelo en worktrees `v2-test-019-*`, sobre los contratos de `specs/019-v1-entrega/design.md`. El orquestador es el único que escribe `docs/` e integra en V2-test. El carril formal arrancó antes de aprobarse la spec 019, por decisión del usuario, porque solo toca `lean/` y `tla/`.
- **El gate deja de ser prosa.** Un hook de Claude Code impide marcar casillas de aprobación y editar `backend/` o `frontend/` sin el plan aprobado ([workflow/3-plan.md](../../workflow/3-plan.md)).
